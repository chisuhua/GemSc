...
void WritebackUnit::tick(uint64_t current_cycle) {
    ...
    while (!queue_.empty()) {
        ...
        if (front.finish_cycle > current_cycle) break;  // 未到时间, 停止 drain
        ...
        if (n > 4) n = 4;  // 防御: 截断到 max
        for (uint8_t i = 0; i < n; ++i) {
            rf->write(front.desc.warpid, front.desc.dst_regs[i], front.desc.result_value[i]);
        }
        queue_.pop_front();
    }
}
